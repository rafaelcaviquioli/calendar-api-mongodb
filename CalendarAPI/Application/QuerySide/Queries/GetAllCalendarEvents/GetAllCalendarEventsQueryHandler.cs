...
namespace CalendarAPI.Application.QuerySide.Queries.GetAllCalendarEvents
{
    public class GetAllCalendarEventsQueryHandler : IRequestHandler<GetAllCalendarEventsQuery, CalendarEventViewModel[]>
    {
        ...
        public async Task<CalendarEventViewModel[]> Handle(GetAllCalendarEventsQuery request, CancellationToken cancellationToken)
            => await _context.CalendarEvents.Select(calendarEvent => new CalendarEventViewModel(
                calendarEvent.Id,
                calendarEvent.Name,
                calendarEvent.Time,
